...
def generate_videos_from_txt_img_mp3(txt_dir, voice_dir, bg_img_dir, output_file, start_second, topic_index, bg_img_ID, bg_type = 0):
    ...
    # PRJ_SCOPIO
    pattern_bg_img = '_bg'   # 之後的背景圖為： 'bg_img_ID'.jpg
    image_filename = os.path.join(bg_img_dir, f"{bg_img_ID}.jpg")
    print("XXXXXXXXX. " + image_filename)
    ...
    for txt_file in all_txt_files:
        ...
        if txt_file.endswith(".txt"):
            ...
            if subtitle_text == pattern_bg_img :
                # skip this txt file & set bg-image file name
                image_filename = os.path.join(bg_img_dir, f"{bg_img_ID}.jpg")
                bg_img_ID = bg_img_ID + 1

                print("===================> using bg-image: " + image_filename)
            ...
            if starts_with_pattern(subtitle_text, pattern_topic):  # topic
                ...
                #PRJ_SCOPIO topic_image_index =  0  # 自topic開始會連續顯示幾頁的image，此變數紀錄目前顯示到第幾頁
                #PRJ_SCOPIO topic_index = topic_index + 1         # 目前處理的是第幾個topic  (根據此變數決定要抓哪張圖, start from 1)
            ...
            # 創建背景 單色或圖片
            #resize_image2(image_filename, 'temp.jpg')    # 處理 '_bg' 時已轉過size
...
topic_num1, video_length1, start_bg_img_ID = \
    generate_videos_from_txt_img_mp3("./腳本/txt"+book_ID+"-1", "./腳本/voice"+book_ID+"-1", "./bg_image/bg"+book_ID,"./output1.mp4",0,0, \
                                         0,bg_type=0)        #4th parameter: start time (in sec.);   bg_type=0 topic背景靜態照片 (==1, topic是動畫)

topic_num2, video_length2, start_bg_img_ID = \
        generate_videos_from_txt_img_mp3("./腳本/txt"+book_ID+"-2", "./腳本/voice"+book_ID+"-2", "./bg_image/bg"+book_ID, "./output2.mp4", \
                                    video_length1,topic_num1, start_bg_img_ID,bg_type=1)

video_length3=video_length2
if clip_number >= 3:
    topic_num3, video_length3, start_bg_img_ID = \
            generate_videos_from_txt_img_mp3("./腳本/txt"+book_ID+"-3","./腳本/voice"+book_ID+"-3", "./bg_image/bg"+book_ID, "./output3.mp4", \
...
if clip_number >= 4:
    topic_num4, video_length4, start_bg_img_ID = \
            generate_videos_from_txt_img_mp3("./腳本/txt"+book_ID+"-4","./腳本/voice"+book_ID+"-4", "./bg_image/bg"+book_ID, "./output4.mp4", \
