    ...
    try:
        ...
        while total_duration < target_duration :
            ...
            for video_file in video_files:
                ...
                clip = FadeIn(1).apply(clip)
                clip = FadeOut(1).apply(clip)
...
# 製作長背景音樂 (from .mp3)
#create_audio_from_mp3s("./bg_mp3", 20*60, "war.mp3")


# 製作長背景影片 (from .mp4)
#create_random_video_from_directory("./主題影片/war", 60*6, "war.mp4")

# 製作長背景影片 (from .jpeg)
#create_video_from_images("./主題圖片/war", 8, 15*60, "war2.mp4")


# step1:  背景圖轉檔
'''
for i in range(0,9):   # range(1:10) <== 1~9
    image_filename = os.path.join('./bg_image/B77', f"{i}.webp")
    #image_filename = os.path.join('./主題圖片/trump', f"{i}.webp")
    convert_webp_to_jpeg_with_resolution(image_filename,(1920, 1080))
'''

# 使用示例
#generate_videos_from_txt_and_mp3("./腳本/txt", "./腳本/voice",  "./bg_image/B67","./output.mp4",0,0)

# step2: 產生影片-1
generate_videos_from_txt_and_mp3("./腳本/txt77-1", "./腳本/voice77-1", "./bg_image/B77","./output1.mp4",0,0)  #4th parameter: start time (in second)

# step3: 產生影片-2
#generate_videos_from_txt_and_mp3("./腳本/txt77-2", "./腳本/voice77-2", "./bg_image/B77", "./output2.mp4",603,5) # 最後一個參數：從第幾個topic開始, start from 0.jpg

# step4: 產生影片-3
#generate_videos_from_txt_and_mp3("./腳本/txt77-3", "./腳本/voice77-3", "./bg_image/B77", "./output3.mp4",1018,7) # 最後一個參數：從第幾個topic開始, start from 0.jpg
