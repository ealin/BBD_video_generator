...
def resize_image2(input_path, output_path, resolution=(1920, 1080)):
    ...
    if original_height > original_width:
        print(" 竖直图像处理")
        new_height = target_height  # 高度调整为 1080
        ...
        # 创建 1920x1080 绿色背景
        canvas = Image.new("RGB", (target_width, target_height), (0, 255, 0))  # 绿色背景
...
BGM_length = 25         # 背景音樂長度 / 單位：分鐘
BGM_output_filename = "covid.mp3"

# function = 2 製作長背景影片 (from .mp4)
A_roll_MP4_length = 26    # A-roll (由MP4組成) 長度 / 單位：分鐘
A_roll_MP4_dir = "startup"   # mp4目錄名稱

# function = 3 製作長背景影片 (from .jpeg)
A_roll_JPEG_length = 20         # A-roll (由JPEG組成) 長度 / 單位：分鐘
Each_JPEG_Length = 13           # 每張JPEG顯示多久時間   / 單位：秒
A_roll_JPEG_dir = "covid"       # .jpeg 目錄名稱
mix_mode = True                 # mix-mode = True : 橫圖/豎圖交雜，則不能做fade in/out

# function = 4  背景圖轉檔 (.webp --> .jpeg)
total_photo_no = 12     # 共有幾張圖(.webp)   e.g. 10 means 0-9
photo_dir = "bg81"

# function 5 generate_videos_from_txt_and_mp3   : test sample

# function 6, generate_videos_from_txt_and_mp3 , 產生影片 1+2 ( or +3)
book_ID = '81'
clip_number = 3    # 總共分為幾段 （e.g. clip_number=3 => B77-1, B77-2, B77-3)
...
# function = 1 製作長背景音樂 (from .mp3)
if function == 1:
    create_audio_from_mp3s("./bg_mp3", BGM_length*60, BGM_output_filename)
...
# function = 3 製作長背景影片 (from .jpeg)
if function == 3:
    create_video_from_images(mix_mode, "./主題圖片/"+A_roll_JPEG_dir, Each_JPEG_Length, 60*A_roll_JPEG_length, A_roll_JPEG_dir+"2.mp4")

# function = 4  背景圖轉檔
if function == 4:
    for i in range(0,total_photo_no):   # range(0:10) <== 0~9
        image_filename = os.path.join("./bg_image/"+photo_dir, f"{i}.webp")
        #image_filename = os.path.join('./主題圖片/trump', f"{i}.webp")
        convert_webp_to_jpeg_with_resolution(image_filename,(1920, 1080))

# function = 5 使用示例
if function == 5:
    generate_videos_from_txt_and_mp3("./腳本/txt", "./腳本/voice",  "./bg_image/bg77","./output.mp4",0,0)

# function = 6  產生影片 1+2 (+3)
if function == 6:
    topic_num1, video_length1 = \
        generate_videos_from_txt_and_mp3("./腳本/txt"+book_ID+"-1", "./腳本/voice"+book_ID+"-1", "./bg_image/bg"+book_ID,"./output1.mp4",0,0)  #4th parameter: start time (in second)
    ...
    if clip_number == 3:
        topic_num3, video_length3 = \
            generate_videos_from_txt_and_mp3("./腳本/txt"+book_ID+"-3", "./腳本/voice"+book_ID+"-3", "./bg_image/bg"+book_ID, "./output3.mp4", \
                                    video_length2,topic_num1+topic_num2) 

    create_audio_from_mp3s("./bg_mp3", video_length3, "output.mp3")

    # 設定 - A_roll_MP4_dir (function-2)
    #create_random_video_from_directory("./主題影片/"+A_roll_MP4_dir, video_length3, A_roll_MP4_dir+".mp4")

    create_video_from_images(mix_mode, "./主題圖片/"+A_roll_JPEG_dir, Each_JPEG_Length, video_length3, A_roll_JPEG_dir+"2.mp4")
