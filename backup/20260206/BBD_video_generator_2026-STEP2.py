...
def generate_videos_from_txt_img_mp3(txt_dir, voice_dir, bg_img_dir, output_file, start_second, topic_index, bg_img_ID, bg_type=0):
    """
    核心功能：從指定目錄讀取 .txt 和 .mp3，生成兩個同步影片：
    1. 背景影片 (_img)
    2. 綠幕字幕影片 (_sub) - 已修正黑框問題，強制所有背景為綠色
    """
    ...
    for txt_file in all_txt_files:
        ...
        if txt_file.endswith(".txt"):
            ...
            if starts_with_pattern(subtitle_text, pattern_topic):
                ...
                # --- 修正重點 ---
                # 將 TextClip 的 bg_color 設定為綠色 (background_color)，而非 None。
                # 這樣即使 TextClip 有邊框或矩形區域，也會是綠色的，與底層融合。
                text_clip = TextClip(
                    text=subtitle_text,
                    font_size=txt_font_size,
                    color=font_color,
                    bg_color=background_color,  # <--- 強制設為綠色，解決黑框問題
...
# Configuration
book_ID = '124'
clip_number = 2         # 總共分為幾段 (B77-1, B77-2, B77-3)
