...
def generate_videos_from_txt_img_mp3(txt_dir, voice_dir, bg_img_dir, output_file, start_second, topic_index, bg_img_ID, bg_type=0):
    ...
    1. 背景影片 (_img)
    2. 綠幕字幕影片 (_sub)
    3. 綠幕頭像影片 (_head) - 採用 PIL 直接合成，解決黑框問題
    """
    ...
    for txt_file in all_txt_files:
        ...
        if txt_file.endswith(".txt"):
            ...
            # --- 頭像影片 (Head) 製作 (PIL 直接合成法) ---
            # 這個方法完全繞過 MoviePy 的合成邏輯，直接用 PIL 畫出一張「綠底+頭像」的圖
            # 然後把這張圖變成影片，絕對不會有黑邊。
            
            # 1. 建立一張全綠色的畫布 (PIL Image)
            # 注意：background_color 格式為 (R, G, B)，PIL 也吃這個格式
            base_img = Image.new("RGB", (1920, 1080), background_color)
            
            # 2. 判斷是否需要貼頭像
            if current_avatar_img is not None:
                try:
                    # 載入頭像，轉為 RGBA 確保有透明通道
                    avatar_pil = Image.open(current_avatar_img).convert("RGBA")
                    
                    # 使用 PIL 的 paste 功能
                    # 參數說明: paste(要貼的圖, 座標, 遮罩)
                    # 第三個參數傳入 avatar_pil 自己，表示使用它自身的 Alpha 通道作為遮罩
                    # 這樣 PIL 會自動處理透明度混合
                    base_img.paste(avatar_pil, current_avatar_pos, avatar_pil)
                    
                except Exception as e:
                    print(f"警告：無法載入頭像圖片 {current_avatar_img}: {e}")
                    # 如果出錯，base_img 仍然是純綠色
            
            # 3. 將合成好的 PIL 圖片轉換為 Numpy Array，再轉為 MoviePy 的 ImageClip
            # 這樣 MoviePy 拿到的是一張已經處理好透明度、完全不透明的 RGB 圖片 (綠底+人頭)
