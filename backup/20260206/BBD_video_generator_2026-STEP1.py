...
def convert_webp_to_jpg_with_resolution(input_file, resolution):
    ...
    :param input_file: str, 輸入 .webp 檔案路徑
    :param resolution: tuple, 目標解析度 (寬度, 高度)
    """
    try:
...
def convert_png_to_jpg_with_resolution(input_file, resolution):
    ...
    :param input_file: str, 輸入 .png 檔案路徑
    :param resolution: tuple, 目標解析度 (寬度, 高度)
    """
    try:
...
def generate_videos_from_txt_img_mp3(txt_dir, voice_dir, bg_img_dir, output_file, start_second, topic_index, bg_img_ID, bg_type=0):
    """
    核心功能：從指定目錄讀取 .txt 和 .mp3，合成帶有字幕與背景的影片。
    
    :param txt_dir: 字幕 txt 檔目錄
    :param voice_dir: 語音 mp3 檔目錄
    :param bg_img_dir: 背景圖片/影片目錄
    :param output_file: 輸出影片檔名
    :param start_second: 影片開始的時間偏移量 (秒)
    :param topic_index: 目前處理的 Topic 索引 (未使用於邏輯判斷，僅作參數傳遞)
    :param bg_img_ID: 背景圖片起始 ID
    :param bg_type: 0 = 靜態背景圖, 1 = 動態背景影片
    """
    ...
    max_line = 0    # 0 代表無限制，大於 0 則用於測試輸出前幾行
    ...
    pattern_bg_img = '_bg'   # 用於切換背景圖的特殊標記
    ...
    for txt_file in all_txt_files:
        ...
        if txt_file.endswith(".txt"):
            ...
            if subtitle_text == pattern_bg_img:
                ...
                resize_image2(image_filename, 'temp.jpg') # 調整圖片大小並存為暫存檔
                continue
            ...
            if starts_with_pattern(subtitle_text, pattern_topic):
                ...
                text_clip = TextClip(
                    ...
                    font=font_ttf,
                    interline=15,
                ).with_position(string_text_align)
            ...
            # --- 背景 Clip 建立 ---
            # 使用先前 resize_image2 產生的 temp.jpg
            bg_clip = ImageClip('temp.jpg') 

            # --- 合成最終片段 ---
            if starts_with_pattern(subtitle_text, pattern_no_show):
                # '!!!!' 開頭：不顯示字幕，只有背景和聲音
...
def create_random_video_from_directory(directory, target_duration, output_file):
    ...
    try:
        ...
        # 裁剪至精確長度 (此邏輯在原始程式碼中被註解掉，這裡保留原始行為，即可能略長)
...
def resize_image2(input_path, output_path, resolution=(1920, 1080)):
    """
    功能：智慧調整圖片大小。
    - 直式圖片：高度縮放至 1080，左右留空填補綠色背景 (便於後製去背或合成)。
    - 橫式圖片：寬度縮放至 1920，高度按比例調整。
    """
...
def create_video_from_images(mix_mode, directory, t1, t2, output_file):
    ...
    :param mix_mode: True = 無轉場, False = 淡入淡出
    """
    try:
        image_files = [os.path.join(directory, f) for f in os.listdir(directory) if f.endswith('.jpg')]
